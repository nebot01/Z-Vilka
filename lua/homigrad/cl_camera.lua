...
function HGAddView(ply, origin, angles, velLen)
	if ply:Alive() then
		...
		walkTime = walkTime + walk * FrameTime() * 1.6 * game.GetTimeScale() * (ply:OnGround() and 1 or 0)
		...
		local x, y = math.cos(huy) * math.sin(huy) * walk * 1.1, math.sin(huy) * walk * 1.1
		local x2, y2 = math.cos(huy) * math.sin(huy) * walk * 1.15 + math.sin(huy + 0.25) * 0.3 * walk, math.sin(huy) * walk * 1.15 + math.cos(huy) * 0.3 * walk

		//angles[1] = angles[1] + x * 1
		//angles[2] = angles[2] + y * 1
		ViewPunch4(Angle(y2 * 1.1, x2 * 1.1, x2 * 35) * 0.0008 * (ishgweapon(wep) and 1.6 or 1.2))
...
hook.Add("HUDPaint", "watermark", function()
	draw.SimpleText("Z-Вилка | commit 5", "Default", ScrW() - 10, ScrH() - 10, Color(255, 255, 255, 180), TEXT_ALIGN_RIGHT, TEXT_ALIGN_BOTTOM)
end)
